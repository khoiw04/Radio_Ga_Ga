...
export default function Social() {
  ...
  const { socialSpecial, isActive, location, state } = usePageContext()
  ...
  return (
    <AnimatePresence mode='popLayout'>
      {isActive && location === '/social' && !state.ShowCredit && state.buttonActive && (
        <motion.div
          key={dimension.width}
          className={`absolute ${!socialSpecial ? 'h-dvh' : 'h-[56dvh]'} w-full items-center justify-center overflow-clip bg-transparent py-2 text-5xl md:relative md:flex md:items-start`}
          initial={{ opacity: 0 }}
          animate={{
            opacity: 1,
            transition: {
          ...
          style={{
            clipPath: dimension.width > 650 ? '' : `polygon(0 30%, 100% 30%, 100% 70%, 0 70%)`
          }}
        >
          {dimension.width > 539 && (
            <>
          ...
          <div
            className='grid h-full w-full grid-cols-2 content-center items-center justify-items-center text-5xl md:relative md:flex md:h-fit md:w-2/3 md:flex-row md:items-start md:justify-between lg:w-1/2 lg:justify-around'
            onMouseOver={() =>
            ...
            {social.map((data, index) => {
              return (
                <motion.a
                  ...
                  onMouseEnter={() => play()}
                  className='font-Social last:hidden focus:outline focus:outline-red-700 last:md:block'
                >
                  {data.name}
                </motion.a>
              )
